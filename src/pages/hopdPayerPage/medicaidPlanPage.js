...
export default class HOPDMedicaidPlanPage extends Component {
    ...
    componentDidMount() {
        ...
        this._service.getPayerPlans({type: 'hopd', category: "medicaid"})
            .then(res => {
            ...
            <>
                <Grid container spacing={4} >
                    ...
                    <Grid item xs={12} md={2} lg={3}>
                        <TotalPlanForm basic={{type: 'hopd', category: 'medicaid'}}></TotalPlanForm>
                    </Grid>
                </Grid>
                { this.state.isOpen ? (<Modal show={this.state.isOpen} onClose={this.toggleModal} class={'small'} title={ this.index !== -1 ? "Edit Payer Plan Data" : "New Payer Plan Data" }>
                    <PlanForm data={this.planToEdit}  basic={{type: 'hopd', category: 'medicaid'}} submit={(data) => this.editData(data)} edit={this.editFlag}></PlanForm>
                </Modal> ) : ''}
            </>
        )
    }
