...
export default class ASCPayerListPage extends Component {
    ...
    componentDidMount() {
        ...
        this._service.getPayerList('asc')
            .then(res => {
        ...
        return (
            <>
                <Grid container spacing={4} >
                    ...
                    <Grid item xs={12}>
                        <Widget title="Payer Coverage ASC List" upperTitle noBodyPadding>
                ...
                {this.state.isOpen ? (<Modal show={this.state.isOpen} onClose={this.toggleModal} class={''} title={this.index !== -1 ? "Edit Payer Plan Data" : "New Payer Plan Data"}>
                    <DetailForm data={this.planToEdit} type={"asc"} submit={(data) => this.editData(data)} edit={this.editFlag}></DetailForm>
                </Modal>) : ''}
            </>
        )
    }
